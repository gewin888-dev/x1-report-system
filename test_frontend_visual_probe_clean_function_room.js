...
(async () => {
  ...
  const stamp='20260503_1834_clean_function_room';
  ...
  const saveRes = await page.evaluate(async () => {
    const payload = {
      project_name: '前台真验-clean_function_room',
      report_number: 'PDJC-BG2026-19WEB-CFR1',
      client_name: '前台真验客户',
      contact_info: '13800000000',
      project_address: '上海市浦东新区测试路1号',
      inspection_area: '前台真验洁净功能用房',
      detection_date: '2026-05-03',
      detection_state: '静态',
      weather: { temperature: '22', humidity: '50', pressure: '1013' },
      domain: 'hospital',
      domain_name: '医院洁净部',
      detection_type: 'clean_function_room',
      detection_type_name: '洁净功能用房',
      basis: ['GB 50333-2013'],
      ...
      rooms: [{
        ...
        summary: {
          ...
          abnormal_items: [
            ...
            { key: 'humidity', value: 80, range: '40~60%', passed: false },
  ...
  const shot = path.join(OUT, `frontend_restore_clean_function_room_${stamp}.png`);
  await page.screenshot({ path: shot, fullPage: true });

  const jsonPath = path.join(OUT, `frontend_restore_clean_function_room_${stamp}.json`);
