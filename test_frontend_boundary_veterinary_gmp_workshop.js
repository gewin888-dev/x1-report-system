...
(async () => {
  ...
  const cases = [
    { id: 'vetgmpA_windspeed_low_out_front', val: '0.35', expectPass: false },
    { id: 'vetgmpA_windspeed_low_eq_front', val: '0.36', expectPass: true },
    { id: 'vetgmpA_windspeed_high_out_front', val: '0.55', expectPass: false },
  ];
  ...
  for (const c of cases) {
    const saveRes = await page.evaluate(async (c) => {
      const payload = {
        ...
        inspection_area: '兽药GMP边界真验',
        detection_date: '2026-05-03',
        detection_state: '静态',
        weather: { temperature: '22', humidity: '50', pressure: '1013' },
        domain: 'pharma',
        domain_name: '制药工业',
        detection_type: 'veterinary_gmp_workshop',
        detection_type_name: '兽药GMP车间',
        basis: ['GB 50591-2010'],
        ...
        rooms: [{
          ...
          level_name: 'A级',
          clean_class: 'A级',
          context: { gmp_grade: 'A级', veterinary: true },
          basis: ['GB 50591-2010'],
          basis_dataset: ['GB 50591-2010'],
          judgement: ['GB 50457-2019'],
          judgement_checked: ['GB 50457-2019'],
          judgement_active: ['GB 50457-2019'],
          judgement_priority: ['GB 50457-2019'],
          params: {
            wind_speed: { values: [c.val], result: '' }
          },
          summary: {
            result_state: c.expectPass ? '合格' : '不合格',
            input_result_state: c.expectPass ? '合格' : '不合格',
            judgement_engine: 'veterinary_gmp_workshop_v1',
            judgement_reason: c.expectPass ? '边界值命中合格区间' : '边界值落在不合格区间',
            judgement_overridden: false,
            abnormal_items: c.expectPass ? [] : [{ key: 'wind_speed', value: Number(c.val), range: '0.36~0.54m/s', passed: false }]
          }
        }],
        inspector: 'admin'
      };
    ...
    const probe = await page.evaluate((c) => {
      ...
      const pb = card?.querySelector('[data-pk="wind_speed"]');
      const resultText = pb?.querySelector('.cv[data-res]')?.textContent?.trim() || '';
      const inputs = Array.from(pb?.querySelectorAll('input') || [])
    ...
    results.push({
      caseId: c.id,
      expectedPass: c.expectPass,
      actualPass: probe.pagePass,
      status: probe.pagePass === c.expectPass ? 'PASS' : 'FAIL',
      probe
    });
  }

  const jsonPath = path.join(OUT, `frontend_boundary_veterinary_gmp_workshop_${stamp}.json`);
