...
(async () => {
  ...
  const cases = [
    { id: 'laminar_airflow_ok', val: '符合要求', expectPass: true },
    { id: 'laminar_airflow_bad', val: '不符合要求', expectPass: false },
  ];
  ...
  for (const c of cases) {
    const saveRes = await page.evaluate(async (c) => {
      const payload = {
        ...
        inspection_area: '层流罩气流流型边界真验', detection_date: '2026-05-03', detection_state: '静态',
        ...
        rooms: [{
          ...
          params: { airflow_pattern: { value: c.val, result: '' } },
          summary: { result_state: c.expectPass ? '合格' : '不合格', input_result_state: c.expectPass ? '合格' : '不合格', judgement_engine: 'laminar_hood_v1', judgement_reason: c.expectPass ? '边界值命中合格区间' : '边界值落在不合格区间', judgement_overridden: false, abnormal_items: c.expectPass ? [] : [{ key: 'airflow_pattern', value: c.val, range: '气流垂直向下、无旋涡', passed: false }] }
        }], inspector: 'admin'
      };
    ...
    const probe = await page.evaluate((c) => {
      ...
      const pb = document.querySelector('[data-pk="airflow_pattern"]');
      ...
      const pagePass = /✅/.test(resultText) || /结果:合格/.test(roomSummaryText);
  ...
  const jsonPath = path.join(OUT, `frontend_boundary_laminar_hood_airflow_pattern_${stamp}.json`);
