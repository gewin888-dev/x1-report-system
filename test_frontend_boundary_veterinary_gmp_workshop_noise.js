...
(async () => {
  ...
  const cases = [
    { id: 'vetgmp_noise_low_in', bg: '40', indoor: '64', expectPass: true },
    { id: 'vetgmp_noise_eq', bg: '40', indoor: '65', expectPass: true },
    { id: 'vetgmp_noise_high_out', bg: '40', indoor: '66', expectPass: false },
  ];
  ...
  for (const c of cases) {
    const saveRes = await page.evaluate(async (c) => {
      const payload = {
        ...
        project_address: '上海市浦东新区测试路1号', inspection_area: '兽药GMP噪声边界真验', detection_date: '2026-05-03', detection_state: '静态',
        ...
        rooms: [{
          ...
          summary: {
            ...
            abnormal_items: c.expectPass ? [] : [{ key: 'noise', value: Number(c.indoor), range: '≤65dB(A)', passed: false }]
          }
        }], inspector: 'admin'
      };
  ...
  const jsonPath = path.join(OUT, `frontend_boundary_veterinary_gmp_workshop_noise_${stamp}.json`);
