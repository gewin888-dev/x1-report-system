...
(async () => {
  ...
  const saveRes = await page.evaluate(async () => {
    const payload = {
      project_name: '前台真验-veterinary_gmp_workshop',
      report_number: 'PDJC-BG2026-19WEB-VGMP1',
      client_name: '前台真验客户',
      contact_info: '13800000000',
      project_address: '上海市浦东新区测试路1号',
      inspection_area: '前台真验兽药GMP车间',
      ...
      rooms: [{
        ...
        params: {
          wind_speed: { values: ['0.55'], result: '不合格' },
        ...
        summary: {
          ...
          abnormal_items: [
            { key: 'wind_speed', value: 0.55, range: '0.36~0.54m/s', passed: false },
            { key: 'pressure', value: 8, range: '≥10Pa', passed: false },
            { key: 'noise', value: 70, range: '≤55dB(A)', passed: false }
          ]
        }
      }],
      inspector: 'admin'
  ...
  const probe = await page.evaluate(() => {
    ...
    return {
      ...
      restoredParams: {
        windSpeedInputs: getInputValues('wind_speed'),
  ...
  const pngName = `frontend_restore_veterinary_gmp_workshop_${stamp}_veterinary_gmp_workshop.png`;
  const jsonName = `frontend_restore_veterinary_gmp_workshop_${stamp}_veterinary_gmp_workshop.json`;
