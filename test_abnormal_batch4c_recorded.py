#!/usr/bin/env python3
"""
X1 异常样本专项记录器（Batch 4c）
目标：按 IVC 当前真实受支持判定键（airchange / airflow_speed / hepa_integrity）重做异常样本，验证是否能稳定返回多项 abnormal_items。
"""
import json
import requests
...
def main():
    ...
    cases = [
        {
            'case_id': 'abnormal_ivc_supported_keys_multi', 'domain': 'biosafety', 'expected_min_abnormal': 3,
            'room': {
                'type_id':'ivc','room_name':'IVC笼具异常样本支持键复测','level_name':'默认','clean_class':'默认',
                'context':{},
                'params': norm_params({
                    'airchange':'8',
                    'airflow_speed':'0.30',
    ...
    out_json = REPORTS / f'abnormal_records_batch4c_{ts}.json'
    out_md = REPORTS / f'abnormal_summary_batch4c_{ts}.md'
    ...
    lines = ['# X1 异常样本专项记录 Batch4c', '', f'- 生成时间：{ts}', f'- 总数：{len(results)}', '']
