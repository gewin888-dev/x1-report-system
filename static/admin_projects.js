...
function getProjectTagClass(type, value){
  ...
  if(['已完成','已签已归档','已回款','已开票','已出具','已发送客户','客户已确认','检测完成'].includes(v)) return 'green';
  if(['检测中','编制中','部分回款','待出具','审核中','已签约','已报价','待客户确认'].includes(v)) return 'blue';
  if(['待签约','合同审批中','未开票','未回款','已排期','待修改'].includes(v)) return 'orange';
...
function renderProjectSummary(summary){ const box=document.getElementById('project-summary-cards'); if(!box) return; const ra=summary.receivable_amount||0; const contractCard=_finPerms.contract?`<div class="stat-card green"><div class="label">合同总额</div><div class="value">${formatMoney(summary.contract_total||0)}</div></div>`:''; const receivableCard=_finPerms.receivable?`<div class="stat-card" style="border-left:4px solid #cf1322;"><div class="label">应收款</div><div class="value" style="color:${ra>0?'#cf1322':'#389e0d'};">${formatMoney(ra)}</div></div>`:''; box.innerHTML=`<div class="stat-cards" style="grid-template-columns:repeat(8,minmax(0,1fr));gap:10px;"><div class="stat-card blue"><div class="label">项目总数</div><div class="value">${summary.project_count||0}</div></div><div class="stat-card orange"><div class="label">检测中</div><div class="value">${summary.detecting_count||0}</div></div><div class="stat-card purple"><div class="label">待出报告</div><div class="value">${summary.pending_report_count||0}</div></div><div class="stat-card blue"><div class="label">未开票</div><div class="value">${summary.pending_invoice_count||0}</div></div><div class="stat-card orange"><div class="label">待回款</div><div class="value">${summary.pending_payment_count||0}</div></div>${contractCard}${receivableCard}<div class="stat-card"><div class="label">已完成</div><div class="value">${summary.done_count||0}</div></div></div>`; }
function loadProjectSummary(){ fetch('/admin/api/business_projects/summary').then(r=>r.json()).then(d=>{ renderProjectSummary(d); }).catch(console.error); }
let currentStatusFilter = ''; function setStatusFilter(status){ currentStatusFilter=status; document.querySelectorAll('.status-filter-btn').forEach(btn=>{ if(btn.dataset.status===status) btn.classList.add('active'); else btn.classList.remove('active'); }); loadProjects(1); } function loadProjects(page=1){ const params=new URLSearchParams(); params.set('page',page); params.set('page_size',20); const m=[['keyword','project-search'],['contract_status','project-contract-status'],['inspection_stage','project-inspection-stage'],['report_status','project-report-status'],['invoice_status','project-invoice-status'],['payment_status','project-payment-status'],['owner','project-owner']]; m.forEach(([k,id])=>{ const v=document.getElementById(id)?.value||''; if(v) params.set(k,v); }); if(currentStatusFilter) params.set('status_group',currentStatusFilter); fetch('/admin/api/business_projects?'+params.toString()).then(r=>r.json()).then(d=>{ if(!d.success) throw new Error(d.error||'加载失败'); renderProjectTable(d); }).catch(err=>{ console.error(err); const box=document.getElementById('project-list'); if(box) box.innerHTML='<div class="empty">加载失败</div>'; }); }
function _classifyProject(x){ const ca=parseFloat(x.contract_amount)||0; const pa=parseFloat(x.paid_amount)||0; const ra=ca-pa; const ins=x.inspection_stage||''; const rpt=x.report_status||''; const inv=x.invoice_status||''; const done_ins=(ins==='检测完成'); const done_rpt=(rpt==='已出报告'||rpt==='已出具'||rpt==='已发送客户'||rpt==='客户已确认'); const done_inv=(inv==='已开票'||inv==='无需开票'); const done_work=done_ins && done_rpt && done_inv; if(done_work && ra<=0) return 'done'; if(done_work && ra>0) return 'receivable'; return 'active'; }
function renderProjectTable(data){ const box=document.getElementById('project-list'); if(!box) return; const items=data.items||[]; const sm=data.summary||{}; const fmtAmt=function(v){v=parseFloat(v)||0;return v?v.toLocaleString('zh-CN',{minimumFractionDigits:0,maximumFractionDigits:2}):'-';}; if(!items.length){ box.innerHTML='<div class="empty">暂无项目</div>'; renderProjectPagination(0,1,20); return; } const renderRow=function(x){ const ca=parseFloat(x.contract_amount)||0; const pa=parseFloat(x.paid_amount)||0; const ra=ca-pa; const statusClass=_classifyProject(x); const statusMap={'active':'🟢 进行中','receivable':'🟡 待收款','done':'✅ 已完结'}; const statusLabel=statusMap[statusClass]||''; const statusColor={'active':'#1677ff','receivable':'#faad14','done':'#52c41a'}[statusClass]||'#666'; return `<tr style="cursor:pointer;transition:background .12s;" onclick="showProjectModal(${x.id},'view')" onmouseenter="this.style.background='#f0f7ff'" onmouseleave="this.style.background=''"><td><span style="color:${statusColor};font-size:12px;margin-right:6px;">${statusLabel}</span>${escapeHtml(x.project_name)}${x.has_urge?'<span title="客户催单" style="color:#f5222d;margin-left:4px;">🔔</span>':''}</td><td style="font-size:11px;color:#666;">${escapeHtml(x.project_no||'-')}</td><td>${escapeHtml(x.client_name)}</td><td>${renderProjectTag('contract_status',x.contract_status)}</td><td style="text-align:right;font-family:monospace;">${_finPerms.contract?(ca?fmtAmt(ca):'-'):'***'}</td><td>${renderProjectTag('inspection_stage',x.inspection_stage)}</td><td>${renderProjectTag('report_status',x.report_status)}</td><td>${renderProjectTag('invoice_status',x.invoice_status)}</td><td style="text-align:right;font-family:monospace;color:#389e0d;">${_finPerms.paid?(pa?fmtAmt(pa):'-'):'***'}</td><td style="text-align:right;font-family:monospace;color:${ra>0?'#cf1322':'#389e0d'};">${_finPerms.receivable?(ca?fmtAmt(ra):'-'):'***'}</td><td>${escapeHtml(x.owner||'-')}</td><td>${escapeHtml((x.updated_at||'').replace('T',' ').slice(0,16)||'-')}</td><td style="text-align:center;"><button class="btn btn-sm" onclick="event.stopPropagation();deleteProject(${x.id})" style="color:#94a3b8;border-color:#e2e8f0;font-size:11px;padding:2px 8px;white-space:nowrap;" onmouseenter="this.style.color='#dc2626';this.style.borderColor='#fca5a5'" onmouseleave="this.style.color='#94a3b8';this.style.borderColor='#e2e8f0'">删除</button></td></tr>`; }; let tbody=items.map(renderRow).join(''); box.innerHTML=`<table><colgroup><col style='width:16%'><col style='width:7%'><col style='width:10%'><col style='width:6%'><col style='width:7%'><col style='width:7%'><col style='width:8%'><col style='width:6%'><col style='width:7%'><col style='width:7%'><col style='width:5%'><col style='width:9%'><col style='width:5%'></colgroup><thead><tr><th>项目名称</th><th>项目编号</th><th>客户/委托单位</th><th>合同情况</th><th style='text-align:right;'>合同金额</th><th>检测阶段</th><th>报告状态</th><th>开票情况</th><th style='text-align:right;'>已收款</th><th style='text-align:right;'>应收款</th><th>负责人</th><th>更新时间</th><th style='text-align:center;'>操作</th></tr></thead><tbody>${tbody}</tbody><tfoot><tr style="background:#f5f5f5;font-weight:600;"><td colspan="4" style="text-align:right;">当前页合计</td><td style="text-align:right;font-family:monospace;">${_finPerms.contract?fmtAmt(sm.contract_amount):'***'}</td><td colspan="3"></td><td style="text-align:right;font-family:monospace;color:#389e0d;">${_finPerms.paid?fmtAmt(sm.paid_amount):'***'}</td><td style="text-align:right;font-family:monospace;color:${(sm.receivable_amount||0)>0?'#cf1322':'#389e0d'};">${_finPerms.receivable?fmtAmt(sm.receivable_amount):'***'}</td><td colspan="3"></td></tr></tfoot></table>`; renderProjectPagination(data.total||0, data.page||1, data.page_size||20); }
...
function renderProjectReportFeedbacks(items){ const box=document.getElementById('project-report-feedback-list'); if(!box) return; if(!items||!items.length){ box.innerHTML='<div style="color:#999;padding:8px 0;">暂无客户报告修正反馈</div>'; return; } box.innerHTML=items.map(function(fb){ const attachments=Array.isArray(fb.attachments)?fb.attachments:[]; const attachmentHtml=attachments.length?'<div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">'+attachments.map(function(att){ return '<a href="'+escapeHtml(att.download_url||'#')+'" target="_blank" rel="noopener noreferrer" style="font-size:12px;padding:4px 8px;border:1px solid #dbeafe;border-radius:999px;background:#eff6ff;color:#1d4ed8;text-decoration:none;">📎 '+escapeHtml(att.original_name||'附件')+'</a>'; }).join('')+'</div>':''; return '<div style="padding:10px 12px;border:1px solid #e5e7eb;border-radius:8px;margin-bottom:8px;background:#fff;">'+'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;gap:8px;">'+'<div style="font-weight:600;color:#111827;font-size:13px;">'+escapeHtml(fb.client_name||'客户')+' · 报告修正反馈</div>'+'<div style="font-size:11px;color:#6b7280;">'+escapeHtml((fb.created_at||'').replace('T',' ').slice(0,16)||'-')+'</div>'+'</div>'+'<div style="font-size:12px;color:#374151;line-height:1.6;white-space:pre-wrap;">'+escapeHtml(fb.content||'')+'</div>'+attachmentHtml+'</div>'; }).join(''); }
function resetProjectForm(){ ['project-id','project-version','project-name','project-client-name','project-address','project-contact-name','project-contact-phone','project-detection-domain','project-detection-type','project-expected-date','project-desc','project-contract-status-form','project-contract-amount','project-paid-amount','project-inspection-stage-form','project-report-status-form','project-invoice-status-form','project-payment-status-form','project-owner-form','project-remarks'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  var reportList = document.getElementById('project-report-list');
  if(reportList) reportList.innerHTML = '<div style="color:#999;">暂无关联报告</div>';
  var reportCount = document.getElementById('project-report-count');
  if(reportCount) reportCount.textContent = '';
  var reportFileList = document.getElementById('report-file-list');
  if(reportFileList) reportFileList.innerHTML = '';
  var reportFileInput = document.getElementById('report-file-input');
  if(reportFileInput) reportFileInput.value = '';
  var reportFileStatus = document.getElementById('report-file-status');
  if(reportFileStatus){ reportFileStatus.style.display = 'none'; reportFileStatus.textContent = ''; }
  var uploadTypeHint = document.getElementById('upload-type-hint');
  if(uploadTypeHint){ uploadTypeHint.textContent = ''; uploadTypeHint.style.color = ''; }
  var caField=document.getElementById('project-contract-amount-field');
  if(caField) caField.style.display=_finPerms.contract?'':'none';
  var paField=document.getElementById('project-paid-amount-field');
  if(paField) paField.style.display=_finPerms.paid?'':'none';
}
function fillProjectForm(x){ document.getElementById('project-id').value=x.id||''; var versionEl=document.getElementById('project-version'); if(versionEl) versionEl.value=(x.version||1); document.getElementById('project-name').value=x.project_name||''; document.getElementById('project-client-name').value=x.client_name||''; document.getElementById('project-address').value=x.project_address||''; document.getElementById('project-contact-name').value=x.contact_name||''; document.getElementById('project-contact-phone').value=x.contact_phone||''; document.getElementById('project-detection-domain').value=x.detection_domain||''; document.getElementById('project-detection-type').value=x.detection_type||''; document.getElementById('project-expected-date').value=x.expected_detection_date||''; document.getElementById('project-desc').value=x.project_desc||''; document.getElementById('project-contract-status-form').value=x.contract_status||'未签'; var caField=document.getElementById('project-contract-amount-field'); var caInput=document.getElementById('project-contract-amount'); if(_finPerms.contract){ if(caField) caField.style.display=''; if(caInput) caInput.value=x.contract_amount||''; } else { if(caField) caField.style.display='none'; } var paField=document.getElementById('project-paid-amount-field'); var paidEl=document.getElementById('project-paid-amount'); if(_finPerms.paid){ if(paField) paField.style.display=''; if(paidEl) paidEl.value=x.paid_amount||''; } else { if(paField) paField.style.display='none'; } document.getElementById('project-inspection-stage-form').value=x.inspection_stage||'未安排'; document.getElementById('project-report-status-form').value=x.report_status||'未开始'; document.getElementById('project-invoice-status-form').value=x.invoice_status||'未开票'; var psf=document.getElementById('project-payment-status-form'); if(psf) psf.value=x.payment_status||'未回款'; document.getElementById('project-owner-form').value=x.owner||''; document.getElementById('project-remarks').value=x.remarks||''; var rfs=document.getElementById('report-file-status'); if(rfs){ rfs.style.display='none'; rfs.textContent=''; } var uth=document.getElementById('upload-type-hint'); if(uth){ var rs=(x.report_status||'').trim(); if(rs==='客户已确认'){ uth.textContent='📌 当前状态"客户已确认"，上传的文件将作为【正式报告（盖章版）】'; uth.style.color='#389e0d'; } else if(rs==='已出报告'){ uth.textContent='✅ 已出报告，流程已结束。如需补传文件将作为正式报告附件'; uth.style.color='#389e0d'; } else if(rs==='待客户确认'){ uth.textContent='📌 当前状态"待客户确认"，客户确认后再上传盖章版'; uth.style.color='#d46b08'; } else { uth.textContent='📌 当前上传将作为【审核稿】，并推进到"待客户确认"'; uth.style.color='#1677ff'; } } }
...
function setProjectFormMode(mode){
  ...
  const isNew = !((document.getElementById('project-id')||{}).value || '');
  ...
  ['project-contract-status-form','project-inspection-stage-form','project-report-status-form','project-invoice-status-form'].forEach(id=>setProjectFieldReadonlyStyle(id, readonly));
  ...
  const uploadTypeHint=document.getElementById('upload-type-hint');
  const reportFileList=document.getElementById('report-file-list');
  const uploadBtn = reportFileInput ? reportFileInput.parentElement?.querySelector('button') : null;
  ...
  if(reportFileInput) reportFileInput.disabled = readonly || isNew;
  if(uploadBtn){
    uploadBtn.disabled = readonly || isNew;
    uploadBtn.style.opacity = (readonly || isNew) ? '0.4' : '1';
    uploadBtn.style.cursor = (readonly || isNew) ? 'not-allowed' : 'pointer';
  }
  if(uploadTypeHint){
    if(isNew){
      uploadTypeHint.textContent = '请先保存项目后再上传检测报告';
      uploadTypeHint.style.color = '#8c8c8c';
    } else if(readonly){
      uploadTypeHint.textContent = '当前为查看模式，仅可查看附件，不可上传或删除';
      uploadTypeHint.style.color = '#8c8c8c';
    }
  }
  if(reportFileList){
    reportFileList.setAttribute('data-readonly', readonly ? '1' : '0');
  }

  // 派单按钮逻辑:有活跃任务时禁用;完成/交付阶段禁用;新项目未保存禁用;其他启用
  ...
  if(dispatchBtn){
    ...
    var rptStatus = (document.getElementById('project-report-status-form')||{}).value || '';
    var projectId = (document.getElementById('project-id')||{}).value || '';
    var doneStages = ['检测完成','已完结','已关闭'];
    var doneReports = ['客户已确认','已发送客户','已出具','已出报告'];
    var isDone = doneStages.indexOf(stage) >= 0 || doneReports.indexOf(rptStatus) >= 0;
    if(isDone || isNew){
      dispatchBtn.disabled = true;
      dispatchBtn.style.opacity = '0.4';
      dispatchBtn.style.cursor = 'not-allowed';
      dispatchBtn.title = isNew ? '请先保存项目' : '项目已进入完成/交付阶段,无需派单';
      dispatchBtn.textContent = isNew ? '+ 派单' : '检测已完成';
    } else {
...
function showProjectModal(id=null, mode='edit'){ const modal=document.getElementById('project-modal'); const title=document.getElementById('project-modal-title'); if(!modal) return; resetProjectForm(); if(id){ title.textContent = mode === 'view' ? '项目信息卡' : '编辑项目'; fetch('/admin/api/business_projects/'+id).then(r=>r.json()).then(d=>{ if(!d.success) throw new Error(d.error||'加载项目失败'); fillProjectForm(d.item||{}); showProjectMeta(d.item||{}); setProjectFormMode(mode); modal.classList.add('show'); loadProjectTasks(id); loadProjectReports(id); loadReportFileList(id); renderProjectReportFeedbacks(d.report_feedbacks || []); }).catch(err=>{ console.error(err); alert(err.message||'加载项目失败'); }); } else { title.textContent='新增项目'; showProjectMeta({}); setProjectFormMode('edit'); renderTaskListEmpty(); renderProjectReportFeedbacks([]); modal.classList.add('show'); } }
function closeProjectModal(){ const modal=document.getElementById('project-modal'); if(modal) modal.classList.remove('show'); resetProjectForm(); setProjectFormMode('edit'); renderTaskListEmpty(); }
function switchProjectModalToEdit(){
  const id = document.getElementById('project-id')?.value || '';
  if(!id) return;
  showProjectModal(id, 'edit');
}
function collectProjectFormData(){ const payload={ project_name:document.getElementById('project-name')?.value||'', client_name:document.getElementById('project-client-name')?.value||'', project_address:document.getElementById('project-address')?.value||'', contact_name:document.getElementById('project-contact-name')?.value||'', contact_phone:document.getElementById('project-contact-phone')?.value||'', detection_domain:document.getElementById('project-detection-domain')?.value||'', detection_type:document.getElementById('project-detection-type')?.value||'', expected_detection_date:document.getElementById('project-expected-date')?.value||'', project_desc:document.getElementById('project-desc')?.value||'', contract_status:document.getElementById('project-contract-status-form')?.value||'', inspection_stage:document.getElementById('project-inspection-stage-form')?.value||'', report_status:document.getElementById('project-report-status-form')?.value||'', invoice_status:document.getElementById('project-invoice-status-form')?.value||'', payment_status:document.getElementById('project-payment-status-form')?.value||'', owner:document.getElementById('project-owner-form')?.value||'', remarks:document.getElementById('project-remarks')?.value||'' }; if(_finPerms.contract){ payload.contract_amount=document.getElementById('project-contract-amount')?.value||0; } if(_finPerms.paid){ payload.paid_amount=document.getElementById('project-paid-amount')?.value||0; } const versionEl=document.getElementById('project-version'); if(versionEl && versionEl.value){ payload.version=Number(versionEl.value)||1; } return payload; }
function saveProject(){ const id=document.getElementById('project-id')?.value||''; const btn=document.getElementById('project-save-btn'); if(btn&&btn.disabled) return; const payload=collectProjectFormData(); if(!payload.project_name.trim()){ alert('项目名称不能为空'); return; } const isEdit=!!id; const oldText=btn?btn.textContent:''; if(btn){ btn.disabled=true; btn.textContent='保存中...'; } fetch(isEdit?('/admin/api/business_projects/'+id):'/admin/api/business_projects',{ method:isEdit?'PUT':'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) }).then(r=>r.json().then(d=>({ok:r.ok,status:r.status,data:d}))).then(({ok,status,data})=>{ if(!ok||!data.success){ if(status===409 || data.code==='version_conflict'){ alert(data.error || '该项目已被其他人修改，请刷新后重试'); if(id){ fetch('/admin/api/business_projects/'+id).then(r=>r.json()).then(d=>{ if(d.success){ fillProjectForm(d.item||{}); showProjectMeta(d.item||{}); } }).catch(()=>{}); } return; } throw new Error(data.error||'保存失败'); } closeProjectModal(); loadProjectSummary(); loadProjects(1); }).catch(err=>{ console.error(err); alert(err.message||'保存失败'); }).finally(()=>{ if(btn){ btn.disabled=false; btn.textContent=oldText||'保存项目'; } }); }
function deleteProject(id){ fetch('/admin/api/business_projects/'+id+'?inspect_only=1').then(r=>r.json()).then(d=>{ if(!d.success) throw new Error(d.error||'获取删除影响失败'); const impact=d.impact||{}; const msg='确认删除项目「'+(impact.project_name||id)+'」？\n\n关联任务：'+(impact.task_count||0)+' 个\n报告附件：'+(impact.attachment_count||0)+' 个\n客户反馈：'+(impact.feedback_count||0)+' 条\n\n删除后将同步删除关联任务；项目主记录将被移除。'; if(!confirm(msg)) return; const btns=[...document.querySelectorAll('button[onclick="event.stopPropagation();deleteProject('+id+')"], button[onclick="deleteProject('+id+')"]')]; btns.forEach(b=>{ b.disabled=true; b.dataset.oldText=b.textContent; b.textContent='删除中...'; }); fetch('/admin/api/business_projects/'+id,{method:'DELETE'}).then(r=>r.json().then(d=>({ok:r.ok,data:d}))).then(({ok,data})=>{ if(!ok||!data.success) throw new Error(data.error||'删除失败'); loadProjectSummary(); loadProjects(1); }).catch(err=>{ console.error(err); alert(err.message||'删除失败'); }).finally(()=>{ btns.forEach(b=>{ b.disabled=false; b.textContent=b.dataset.oldText||'删除'; }); }); }).catch(err=>{ console.error(err); alert(err.message||'删除前检查失败'); }); }
...
function showProjectMeta(meta){ var box=document.getElementById('project-meta-info'); if(!box) return; var updatedAt=(meta.updated_at||'').replace('T',' ').slice(0,16)||'-'; var updatedBy=meta.updated_by_name||meta.updated_by||'-'; box.innerHTML='最后更新：'+escapeHtml(updatedBy)+' · '+escapeHtml(updatedAt); }
...
function renderProjectTaskList(items, projectId){
  const box = document.getElementById('project-task-list');
  if(!box) return;
  const projectStage = (document.getElementById('project-inspection-stage-form')||{}).value || '';
  const projectReportStatus = (document.getElementById('project-report-status-form')||{}).value || '';
  if(!items.length){
  ...
  let html = '';
  items.forEach(t => {
    const projectDone = ['检测完成','已完结','已关闭'].indexOf(projectStage) >= 0 || ['客户已确认','已发送客户','已出具','已出报告'].indexOf(projectReportStatus) >= 0;
    const canCancel = !projectDone && t.task_status !== 'completed' && t.task_status !== 'cancelled';
    ...
    html += `<div style="color:#6b7280;">最后更新</div><div style="grid-column:span 3;">${escapeHtml((t.updated_by_name||t.updated_by||'-'))} · ${escapeHtml((t.updated_at||'').replace('T',' ').slice(0,16)||'-')}</div>`;
  ...
  if(dispBtn){
    ...
    if(hasActive){
      ...
      dispBtn.title = '已有进行中的任务';
      dispBtn.textContent = '检测进行中';
    } else {
      dispBtn.disabled = false;
      dispBtn.style.opacity = '1';
      dispBtn.style.cursor = 'pointer';
      dispBtn.title = '';
      dispBtn.textContent = '+ 派单';
    }
  }
}
...
function showDispatchDialog(){
  ...
  const mode = document.getElementById('project-modal-mode')?.value || 'edit';
  const stage = (document.getElementById('project-inspection-stage-form')||{}).value || '';
  const rptStatus = (document.getElementById('project-report-status-form')||{}).value || '';
  const dispatchBtn = document.getElementById('project-dispatch-btn');
  const doneStages = ['检测完成','已完结','已关闭'];
  const doneReports = ['客户已确认','已发送客户','已出具','已出报告'];
  const isDone = doneStages.indexOf(stage) >= 0 || doneReports.indexOf(rptStatus) >= 0;
  if(!projectId){ alert('请先保存项目'); return; }
  if(mode === 'view'){ alert('查看模式下不可发起派单'); return; }
  if(isDone){ alert('项目已进入完成/交付阶段，无需派单'); return; }
  if(dispatchBtn && dispatchBtn.disabled){
    alert(dispatchBtn.title || '当前不可发起派单');
    return;
  }
  // 加载检测员列表
...
function submitDispatch(){
  ...
  if(submitBtn){ submitBtn.disabled=true; submitBtn.textContent='派单中...'; }
  fetch('/admin/api/project_tasks', {
    ...
    }).finally(()=>{ if(submitBtn){ submitBtn.disabled=false; submitBtn.textContent=oldText||'确认派单'; } });
}
...
function cancelProjectTask(taskId, projectId){
  ...
  fetch('/admin/api/project_tasks/' + taskId + '/cancel', {
    ...
    }).finally(()=>{ if(btn){ btn.disabled=false; btn.textContent=oldText||'取消任务'; } });
}
...
function uploadReport() {
  ...
  var uploadBtn = fileInput ? fileInput.parentElement?.querySelector('button') : null;
  var oldText = uploadBtn ? uploadBtn.textContent : '';
  ...
  }).finally(function(){ if(uploadBtn){ uploadBtn.disabled=false; uploadBtn.textContent=oldText||'上传报告'; } });
}
  ...
  fetch('/admin/api/business_projects/' + projectId + '/report_files', { credentials: 'same-origin' })
    ...
    .then(function(d) {
      ...
      d.files.forEach(function(f) {
        ...
        var readonly = container.getAttribute('data-readonly') === '1';
