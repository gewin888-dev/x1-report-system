...
const REPORT_STATUS_OPTIONS = ['未开始','编制中','审核中','待修改','待出具','已出具','已发送客户'];
...
function getProjectTagClass(type, value){
  ...
  if(['已完成','已签已归档','已回款','已开票','已出具','已发送客户'].includes(v)) return 'green';
  if(['检测中','编制中','待开票','部分回款','待出具','审核中','已签约','已报价'].includes(v)) return 'blue';
...
function _classifyProject(x){ const ca=parseFloat(x.contract_amount)||0; const pa=parseFloat(x.paid_amount)||0; const ra=ca-pa; const ins=x.inspection_stage||''; const rpt=x.report_status||''; const inv=x.invoice_status||''; const done_ins=(ins==='检测完成'||ins==='已结束'); const done_rpt=(rpt==='已出具'||rpt==='已发送客户'); const done_inv=(inv==='已开票'||inv==='无需开票'); const done_work=done_ins && done_rpt && done_inv; if(done_work && ra<=0) return 'done'; if(done_work && ra>0) return 'receivable'; return 'active'; }
