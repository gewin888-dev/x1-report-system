...
(async () => {
  ...
  const cases = [
    { id: 'laminar_uniformity_low_in', values: ['0.40','0.50'], expectPass: true },
    { id: 'laminar_uniformity_eq', values: ['0.40','0.60'], expectPass: true },
    { id: 'laminar_uniformity_high_out', values: ['0.40','0.70'], expectPass: false },
  ];
  ...
  for (const c of cases) {
    const saveRes = await page.evaluate(async (c) => {
      const payload = {
        project_name: c.id, report_number: c.id, client_name: '边界测试单位', contact_info: '13800000000', project_address: '上海市浦东新区测试路1号',
        inspection_area: '层流罩风速均匀度边界真验', detection_date: '2026-05-03', detection_state: '静态',
        weather: { temperature: '22', humidity: '50', pressure: '1013' }, domain: 'pharma', domain_name: '制药工业', detection_type: 'laminar_hood', detection_type_name: '层流罩', basis: ['GB 50591-2010'], judgement: ['GB 50591-2010'],
        rooms: [{
          ...
          params: { avg_speed: { values: c.values, result: '' } },
          summary: { result_state: c.expectPass ? '合格' : '不合格', input_result_state: c.expectPass ? '合格' : '不合格', judgement_engine: 'laminar_hood_v1', judgement_reason: c.expectPass ? '边界值命中合格区间' : '边界值落在不合格区间', judgement_overridden: false, abnormal_items: c.expectPass ? [] : [{ key: 'speed_uniformity', value: 0.27, range: '≤0.20', passed: false }] }
        }], inspector: 'admin'
      };
    ...
    const probe = await page.evaluate((c) => {
      ...
      const pb = document.querySelector('[data-pk="speed_uniformity"]') || document.body;
  ...
  const jsonPath = path.join(OUT, `frontend_boundary_laminar_hood_speed_uniformity_${stamp}.json`);
