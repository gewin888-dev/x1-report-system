...
(async () => {
  ...
  const cases = [
    { id: 'laminar_hepa_eq', val: '0.01', expectPass: true },
    { id: 'laminar_hepa_high_out', val: '0.02', expectPass: false },
    { id: 'laminar_hepa_low_in', val: '0.00', expectPass: true },
  ];
  ...
  for (const c of cases) {
    const saveRes = await page.evaluate(async (c) => {
      const payload = {
        ...
        inspection_area: '层流罩高效过滤器检漏边界真验', detection_date: '2026-05-03', detection_state: '静态',
        ...
        rooms: [{
          ...
          params: { hepa_leak: { values: [c.val], result: '' } },
          summary: { result_state: c.expectPass ? '合格' : '不合格', input_result_state: c.expectPass ? '合格' : '不合格', judgement_engine: 'laminar_hood_v1', judgement_reason: c.expectPass ? '边界值命中合格区间' : '边界值落在不合格区间', judgement_overridden: false, abnormal_items: c.expectPass ? [] : [{ key: 'hepa_leak', value: Number(c.val), range: '≤0.01%', passed: false }] }
        }], inspector: 'admin'
      };
    ...
    const probe = await page.evaluate((c) => {
      ...
      const pb = document.querySelector('[data-pk="hepa_leak"]');
  ...
  const jsonPath = path.join(OUT, `frontend_boundary_laminar_hood_hepa_leak_${stamp}.json`);
