#!/usr/bin/env python3
"""
X1 异常样本专项记录器（Batch 2b）
目标：修正 batch2 中 animal_room / pass_box / ivc 的样本口径，验证其是否能真实进入判定引擎。
"""
import json
import requests
...
def run_case(case):
    ...
    return {
        ...
        'ok': bool(body.get('export_id')) and summary.get('result_state') == '不合格' and len(abnormal_items) >= case['expected_min_abnormal'] and bool(summary.get('judgement_engine')) and summary.get('judgement_engine') != 'unmatched_or_insufficient_params',
    }
...
def main():
    ...
    cases = [
        {
            'case_id': 'abnormal_animal_barrier_pressure', 'domain': 'biosafety', 'expected_min_abnormal': 1,
            'room': {
                'type_id':'animal_room','room_name':'屏障环境饲养室异常样本','level_name':'屏障环境','clean_class':'屏障环境',
                'context':{'animal_environment':'屏障环境','barrier_room_class':'饲养室'},
                'params': norm_params({'pressure': {'values':['2']}, 'temperature': {'values':['35']}}),
                'summary':{'result_state':'合格'}
            }
        },
        {
            'case_id': 'abnormal_passbox_noise', 'domain': 'pharma', 'expected_min_abnormal': 1,
            'room': {
                'type_id':'pass_box','room_name':'传递窗异常样本','level_name':'默认','clean_class':'默认',
                'context':{},
                'params': norm_params({'noise': {'values':['80']}, 'hepa_leak': {'values':['0.05']}}),
                'summary':{'result_state':'合格'}
            }
        },
        {
            'case_id': 'abnormal_ivc_airchange', 'domain': 'biosafety', 'expected_min_abnormal': 1,
            'room': {
                'type_id':'ivc','room_name':'IVC笼具异常样本','level_name':'默认','clean_class':'默认',
                'context':{},
                'params': norm_params({'airchange': {'values':['10']}, 'airflow_speed': {'values':['0.30']}}),
                'summary':{'result_state':'合格'}
            }
        },
    ]
    ...
    out_json = REPORTS / f'abnormal_records_batch2b_{ts}.json'
    out_md = REPORTS / f'abnormal_summary_batch2b_{ts}.md'
    ...
    lines = ['# X1 异常样本专项记录 Batch2b', '', f'- 生成时间：{ts}', f'- 总数：{len(results)}', '']
