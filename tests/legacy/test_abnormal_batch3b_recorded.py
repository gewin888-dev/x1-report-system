#!/usr/bin/env python3
"""
X1 异常样本专项记录器（Batch 3b）
目标：纠偏 batch3 中 operating_room 样本口径，验证其是否能真实进入判定引擎并稳定返回不合格。
"""
import json
import requests
...
def main():
    ...
    cases = [
        {
            'case_id': 'abnormal_or_level2_multi_fix', 'domain': 'hospital', 'expected_min_abnormal': 3,
            'room': {
                'type_id':'operating_room','room_name':'千级手术室异常样本纠偏','level_name':'Ⅱ级','clean_class':'Ⅱ级',
                'context':{'room_type':'main-room','clean_class_code':'level2'},
                'params': norm_params({
                    'airchange':'20','pressure':'4','temperature':'27','humidity':'72','noise':'54',
    ...
    out_json = REPORTS / f'abnormal_records_batch3b_{ts}.json'
    out_md = REPORTS / f'abnormal_summary_batch3b_{ts}.md'
    ...
    lines = ['# X1 异常样本专项记录 Batch3b', '', f'- 生成时间：{ts}', f'- 总数：{len(results)}', '']
