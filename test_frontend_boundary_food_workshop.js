...
(async () => {
  ...
  const cases = [
    { id: 'food_airchange_low_out', val: '14', expectPass: false },
    { id: 'food_airchange_eq', val: '15', expectPass: true },
    { id: 'food_airchange_high_in', val: '16', expectPass: true },
  ];
  ...
  for (const c of cases) {
      ...
      const payload = {
        ...
        rooms: [{
          ...
          summary: {
            ...
            abnormal_items: c.expectPass ? [] : [{ key: 'airchange', value: Number(c.val), range: '≥15次/h', passed: false }]
          }
        }],
        inspector: 'admin'
      };
    ...
    const probe = await page.evaluate((c) => {
      ...
      const pb = card?.querySelector('[data-pk="airchange"]');
  ...
  const jsonPath = path.join(OUT, `frontend_boundary_food_workshop_${stamp}.json`);
